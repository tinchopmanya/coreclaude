...
test("sandbox project flow", async () => {
  ...
  try {
    ...
    const detail = await fetch(`http://127.0.0.1:${frontendPort}/api/projects/${createdProject.id}`, { headers: { cookie } });
    assert.equal(detail.status, 200);

    const deleted = await fetch(`http://127.0.0.1:${frontendPort}/api/projects/${createdProject.id}`, {
      method: "DELETE",
      headers: { cookie }
    });
